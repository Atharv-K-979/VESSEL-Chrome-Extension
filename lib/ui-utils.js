...
export function showRedactionModal(field, originalText, matches) {
  ...
  const uniqueMatches = [...new Set(matches.map(m => m.name || m.type))];

  overlay.innerHTML = `
        <div class="modal-content">
            <h3>Sensitive Data Warning</h3>
            <p>VESSEL detected ${matches.length} sensitive item(s):</p>
            <ul>
                ${uniqueMatches.map(m => `<li>🔴 ${escapeHtml(m)}</li>`).join('')}
            </ul>
            <div class="buttons">
